...
@section('js')
<script>
    $('#invcasino_nav').removeClass("closed").addClass("active").addClass("expand")
    ...
    function elim(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('invcasino.destroy', 'id_replace' ) }}".replace('id_replace', id);
                $.ajax({
                    url: url,
                    type: "DELETE",
                    data: {
    ...
    function edit_Submit(id) {
        ...
        let url = "{{ route('invcasino.update', 'id_replace' ) }}".replace('id_replace', id);
        $.ajax({
            url: url,
            type: "PUT",
            data: {
    ...
    dataTable("{{route('invcasino.service')}}",[
        { data: 'id' },
        {
        ...
        /* {
            render: function ( data,type, row  ) {
                ...
                let url_edit = "{{ route('invcasino.edit', 'id_replace' ) }}".replace('id_replace', row.id);
                let url_destroy = "{{ route('invcasino.destroy', 'id_replace' ) }}".replace('id_replace', row.id);
                return `
