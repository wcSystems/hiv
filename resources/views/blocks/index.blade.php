...
@section('js')
<script>
    $('#blocks_nav').removeClass("closed").addClass("active").addClass("expand")
    ...
    function elim(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = "{{ route('blocks.destroy', 'id_replace' ) }}".replace('id_replace', id);
                $.ajax({
                    url: url,
                    type: "DELETE",
                    data: {
    ...
    function edit_Submit(id) {
        ...
        let url = "{{ route('blocks.update', 'id_replace' ) }}".replace('id_replace', id);
        $.ajax({
            url: url,
            type: "PUT",
            data: {
    ...
    dataTable("{{route('blocks.service')}}",[
        { data: 'id' },
        { data: 'name' },
        {
            render: function ( data,type, row  ) {
                data_modal_current[row.id] = row
                let url_edit = "{{ route('blocks.edit', 'id_replace' ) }}".replace('id_replace', row.id);
                let url_destroy = "{{ route('blocks.destroy', 'id_replace' ) }}".replace('id_replace', row.id);
                return `
